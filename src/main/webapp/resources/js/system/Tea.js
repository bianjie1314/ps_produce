(function($) {
    ...
    function initUI() {
        var table = $('#datatable').DataTable({
            ...
            "sAjaxSource" : ctx + '/system/user/source/13',
       ...
       toolbar.append('<li><a href="#" id="addCourse">分配课程</a></li>');
       ...
       $('#edit').click(function() {
           ...
           var data = datas[0];
           window.location.href = ctx + "/system/user/form/13?id="+data.id;
       ...
       $('#addCourse').click(function() {
   	   ...
   	   $f.dialog({
              ...
              buttons : {
                  "确定" : function() {
               	      var course =  $f.find('[dialog-id=course]').val();
                      var param = {
                   		teaId:data.id,
                   		courseId:course
                      };
                      $.ajax({
                          url : ctx + "/system/user/distribution",
                          type : "POST",
                          contentType : 'application/json; charset=utf-8',
                          dataType : 'json',
                          data : JSON.stringify(param),
