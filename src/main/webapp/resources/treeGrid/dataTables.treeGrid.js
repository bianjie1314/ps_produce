...
(function (factory) {
    ...
    $.extend(TreeGrid.prototype, {
        fnConstruct: function (oInit) {
            ...
            var hideRow =function (subRows,nodes) {
            	if (subRows&& subRows.length){
            		 subRows.forEach(function (node) {
                      	var row=dataTable.row($(node));
                      	 var data = row.data();
                      	var t =$(node).find('.treegrid-control-open');
                      	nodes.push(node);
                    	if(t.length>0){
                         	 hideRow(treeGridRows[row.index()],nodes);
                      	}
            ...
            var resetTreeGridRows = function (index) {
                ...
                indexes.forEach(function (index) {
                    ...
                    if (subRows && subRows.length) {
                    	var nodes=[];
                    	 hideRow(subRows,nodes);
                    	nodes.forEach(function (node) {
                    		dataTable.row($(node)).child.hide();
                        ...
                        /*subRows.forEach(function (node) {
                        	var row=dataTable.row($(node));
                        	var index=row.index();
                        	 var subRows2 = treeGridRows[index];
                        	dataTable.row($(node)).child.hide();
                            dataTable.row($(node)).remove();
                            $(node).remove();
                        });*/
